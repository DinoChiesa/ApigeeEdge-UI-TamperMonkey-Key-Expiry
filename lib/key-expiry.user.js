...
(function (globalScope){
  ...
  function getElementsByTagAndClass(root, tag, clazz) {
    ...
    if (tag) {
      ...
      nodes = Array.prototype.filter.call(nodes, function(testElement){
        return testElement.nodeName === tagUpper;
      });
    }
    return nodes;
  }
  ...
  function addNewAppDetailsLineItem(form, item, appdetails) {
    var newDiv = document.createElement("div");
    newDiv.setAttribute("class", "control-group");
    var newLabel = document.createElement("label");
    newLabel.setAttribute("class", "control-label");
    var newContent = document.createTextNode("Status");
    ...
    newContent = document.createTextNode(status);
    span.appendChild(newContent);
    childDiv.appendChild(span);
    form.insertBefore(newDiv, item.nextElementSibling.nextElementSibling);
  }
        ...
        if (items && items[0]) {
          ...
          if (rows && rows[0]) {
            ...
            if (heads && heads[2]) {
              ...
              var newContent = document.createTextNode("Expiry");
              newHead.appendChild(newContent);
              rows[0].insertBefore(newHead, heads[2]);

              // now add an element to each row in the tbody there-after
                    ...
                    if (cells) {
                      ...
                      if (appdetails && appdetails.credentials && appdetails.credentials[0]) {
                        if (appdetails.credentials[0].expiresAt) {
                          ...
                          newContent = document.createTextNode(dateFormat(d, dateFormatString));
                          span.appendChild(newContent);
                          ...
                          else if (now > appdetails.credentials[0].expiresAt - 86400 * 1000 * 10) {
                            span.appendChild(document.createElement("br"));
                          ...
                          newContent = document.createTextNode("-never-");
                          span.appendChild(newContent);
                        }
